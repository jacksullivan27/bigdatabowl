...
classify_motion <- function(motion_data) {
  ...
  motion_segment <- motion_data %>%
    filter(event %in% c("man_in_motion", "ball_snap") | between(frameId, 
  ...
  motion_summary <- motion_data %>%
    summarise(
      ...
      # Check if player goes behind RB (5 yards behind QB)
      behind_rb = any((x - (qb_snap_x - 5)) * adjust_x < 0),
  ...
  motion_type <- case_when(
    ...
    motion_summary$offenseFormation %in% c("SHOTGUN", "EMPTY", "PISTOL", "WILDCAT") & 
    ...
    motion_summary$offenseFormation %in% c("I_FORM", "SINGLEBACK") & 
      motion_summary$behind_qb & !motion_summary$behind_rb & 
      motion_summary$crossed_center ~ "Fly",
    motion_summary$offenseFormation %in% c("I_FORM", "SINGLEBACK") & 
      motion_summary$behind_qb & !motion_summary$behind_rb & 
      !motion_summary$crossed_center ~ "Jet",
    
    # Lateral motions (using y-coordinate changes for sideline movement)
    abs(motion_summary$delta_y) > abs(motion_summary$delta_x) & 
      motion_summary$delta_y > 0 ~ "Out",  # Toward sideline
    abs(motion_summary$delta_y) > abs(motion_summary$delta_x) & 
      motion_summary$delta_y < 0 ~ "In",   # Toward field
