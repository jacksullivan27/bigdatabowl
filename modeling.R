...
train_data <- train_data %>%
  filter( !is.na(offenseFormation) &
            !is.na(receiverAlignment)  & !is.na(motion_type) & !is.na(down) & !is.na(pff_manZone)
  )
test_data <- test_data %>%
  filter(!is.na(offenseFormation) &
            !is.na(receiverAlignment)  & !is.na(motion_type) & !is.na(down) & !is.na(pff_manZone)
...
# Combine dummies with numeric features
...
params <- list(
  ...
  eta = 0.3,                     # Default learning rate                    # No minimum loss reduction required
